...
public class BasketTest {
    ...
    void consolidateBasketTest() {
        ...
        basket.add("productCode3", "myProduct3", 15);

        basket.consolidateItems();
        List<BasketItem> basketItems = basket.getItems();

        assertEquals(basketItems.size(),3);
        assertEquals(basketItems.get(0).getProductCode(), "productCode");
        assertEquals(basketItems.get(0).getQuantity(), 10);
        assertEquals(basketItems.get(1).getProductCode(), "productCode2");
        assertEquals(basketItems.get(1).getQuantity(), 45);
        assertEquals(basketItems.get(2).getProductCode(), "productCode3");
        assertEquals(basketItems.get(2).getQuantity(), 15);
    }
}
